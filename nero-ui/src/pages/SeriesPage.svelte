...
<div class="grid h-full grid-cols-[2fr_3fr] bg-white">
  <figure class="overflow-hidden">
    {#if seriesQuery.isLoading}
      <p class="text-neutral-600">Loading...</p>
    {:else if seriesQuery.isError}
      <div class="size-full rounded-xl bg-gray-200"></div>
  ...
  <article
    class="flex flex-col justify-center gap-4 overflow-y-auto bg-white px-8
      pt-4"
  >
    {#if seriesQuery.isLoading}
      <p class="text-neutral-600">Loading...</p>
        ...
        <div class="flex w-full items-center gap-6 py-4">
          <button
            class="flex flex-1 cursor-pointer items-center justify-center gap-2.5
              rounded-lg bg-neutral-800 px-12 py-2 text-sm
              text-neutral-100 transition-colors hover:bg-neutral-800"
          ...
          <button
            class="flex size-10 shrink-0 cursor-pointer items-center justify-center
              rounded-full border-[1.5px] border-neutral-300 text-neutral-700
          ...
          <button
            ...
            title="Like"
          >
            <ThumbsUpIcon size={20} strokeWidth={2} />
          </button>
        </div>
      </header>
    {/if}
    ...
    <Tabs.Root value="episodes">
      <Tabs.List
        class="flex w-full justify-between border-t border-neutral-200"
      >
        <Tabs.Trigger
          value="episodes"
          class="tab-trigger flex-1 cursor-pointer py-2.5 text-center text-sm
            ...
            data-[state=active]:border-neutral-900
      ...
      <Tabs.Content value="episodes" class="pt-2">
        {#if episodesQuery.isLoading}
          <p class="text-center text-sm text-neutral-500">Loading...</p>
        ...
        {:else if episodesQuery.isSuccess}
          ...
          {#if episodesQuery.isFetchingNextPage}
            <p class="p-2 text-center text-sm text-neutral-500">
              Loading more...
            </p>
          {/if}
        {/if}
      ...
      <Tabs.Content value="more" class="text-neutral-500">
