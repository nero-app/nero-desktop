<script lang="ts">
  ...
  type NodeConfig =
    ...
    | { type: "custom"; node: string; npm: string };

  const initMutation = createMutation(async (config: NodeConfig) => {
  ...
  <div class="flex flex-col gap-4 p-4">
    ...
    <button
      ...
      class={`w-full rounded-md px-4 py-2 text-sm font-medium text-white
        transition-colors disabled:cursor-not-allowed disabled:bg-neutral-300 ${
          initMutation.isLoading
